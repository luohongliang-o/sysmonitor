...
#import <dll/msado15.dll> rename("EOF", "EndOfFile")
...
public:
	...
	// opander add
	BOOL AddInputParameter(CString strName, int nValue);
	BOOL AddInputParameter(CString strName, long lValue);
	BOOL AddInputParameter(CString strName, long lSize, CString strValue);
	BOOL DelParameter(LONG nIndex);
	BOOL ResetParameter();
...
public:
	...
	void SetErrorMessage(LPCSTR lpstrError = _T(""))		{m_strError = lpstrError;};

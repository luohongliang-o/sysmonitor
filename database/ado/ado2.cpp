...
BOOL CJetEngine::RefreshCache(ADODB::_Connection *pconn)
{
	...
	try
	{
		::CoInitialize(NULL);
		IJetEnginePtr jet(__uuidof(JetEngine)); 
		HRESULT hr = jet->RefreshCache((ADODB::_Connection_Deprecated*)pconn);
		::CoUninitialize();
		return hr == S_OK;
	}
	catch(_com_error) 
...
BOOL CADORecordset::Requery()
{
	if(IsOpen())
	{
		try
		{
		// 如果提示adExecuteRecord无定义，肯定是msado15.dll版本不对
...
BOOL CADOCommand::AddInputParameter(CString strName, int nValue)
{
	return AddParameter(strName, CADORecordset::typeInteger, CADOParameter::paramInput, sizeof(int), nValue);
}

BOOL CADOCommand::AddInputParameter(CString strName, long lValue)
{
	return AddParameter(strName, CADORecordset::typeBigInt, CADOParameter::paramInput, sizeof(long), lValue);
}

BOOL CADOCommand::AddInputParameter(CString strName, long lSize, CString strValue)
{
	return AddParameter(strName, CADORecordset::typeChar, CADOParameter::paramInput, lSize, strValue);
}

BOOL CADOCommand::DelParameter(LONG nIndex)
{
	if(m_pCommand!=NULL)
	{
		_variant_t index=(long)nIndex;
		m_pCommand->Parameters->Delete(index);
	}
	return TRUE;
}

BOOL CADOCommand::ResetParameter()
{
	if(m_pCommand!=NULL)
	{
		long nParamcount = m_pCommand->Parameters->GetCount();
		for (int nIndex_ = 0; nIndex_<nParamcount; nIndex_++)
		{
